...
reservation_seat_table = Table('reservation_seat', Base.metadata,
                               ...
                               Column('seat_id', Integer, ForeignKey(
                                   'seats.id', ondelete='CASCADE'), primary_key=True)
                               )
...
class Reservation(Base):
    ...
    showtime_id = Column(Integer, ForeignKey(
        'showtimes.id', ondelete='CASCADE'), nullable=False)
    ...
    seats = relationship(
        "Seat", secondary=reservation_seat_table, backref="reservations")
