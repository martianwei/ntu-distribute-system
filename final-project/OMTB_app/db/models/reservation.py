...
from db.models.showtime import Showtime, ShowtimeSchema
...
reservation_seat_table = sa.Table(
    ...
    sa.Column('reservation_id', sa.Integer, sa.ForeignKey(
        'reservations.id', ondelete='CASCADE'), primary_key=True),
    sa.Column('seat_id', sa.Integer, sa.ForeignKey(
...
class Reservation(Base):
    ...
    showtime_id = sa.Column(sa.Integer, sa.ForeignKey(
...
def check_seats_can_reserved(showtime_id: int, want_seats: list[int]) -> bool:
    reserved_seats = session.query(Reservation).filter_by(
        showtime_id=showtime_id, status="SUCCESS").all()
    reserved_seats_ids = [
        seat.id for reservation in reserved_seats for seat in reservation.seats]
